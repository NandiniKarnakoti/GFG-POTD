# Problem: Adventure in a Maze
# Difficulty: Hard
# Date: 12 August 2026

"""
Problem:
Given an n x n maze where each cell contains:

1 -> Move Right
2 -> Move Down
3 -> Move Right or Down

Find:
1. Total number of valid paths from (0, 0) to (n-1, n-1)
2. Maximum Adventure among all valid paths
"""

# ---------------------------------------------------
# Approach: Dynamic Programming
# Time Complexity: O(N²)
# Space Complexity: O(N²)
# ---------------------------------------------------

class Solution:
    def findWays(self, grid):
        # code here
        n=len(grid)
        MOD=10**9+7
        ways=[[0]*n for _ in range(n)]
        best=[[0]*n for _ in range(n)]
        ways[0][0]=1
        best[0][0]=grid[0][0]
        for i in range(n):
            for j in range(n):
                if ways[i][j]==0:
                    continue
                if grid[i][j] in (1,3) and j+1<n:
                    ways[i][j+1]=(ways[i][j+1]+ways[i][j])%MOD
                    best[i][j+1]=max(best[i][j+1],best[i][j]+grid[i][j+1])
                if grid[i][j] in (2,3) and i+1<n:
                    ways[i+1][j]=(ways[i+1][j]+ways[i][j])%MOD
                    best[i+1][j]=max(best[i+1][j],best[i][j]+grid[i+1][j])
        return [ways[n-1][n-1],best[n-1][n-1]]
